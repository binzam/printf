...
cxrs += _memcpy(output, &plus, 1);
if (ZERO_FLAG == 1 && d < 0)
cxrs += _memcpy(output, &neg, 1);
pad = (ZERO_FLAG == 1) ? '0' : ' ';
for (wid -= count; wid > 0; wid--)
cxrs += _memcpy(output, &pad, 1);
}
if (ZERO_FLAG == 0 && d < 0)
cxrs += _memcpy(output, &neg, 1);
...
cxrs += _memcpy(output, &plus, 1);
if (!(d == 0 && prec == 0))
cxrs += convert_sbase(output, d, "0123456789", flags, 0, prec);
...
if (!(num == 0 && prec == 0))
cxrs += convert_ubase(output, num, "01234567",
flags, wid, prec);
...
if (!(num == 0 && prec == 0))
cxrs += convert_ubase(output, num, "0123456789",
flags, wid, prec);
