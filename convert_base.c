...
cxrs += convert_sbase(output, num / size, base,
flags, wid - 1, prec - 1);
 ...
 *                 stores the result to the buffer
 * @output: A buffer_t struct containing a character array.
 * @num: An unsigned long
 * @base: A pointer to a string containing the base to convert
 * @flags: Flag
 * @wid: width
 * @prec: precision
 *
...
if (num >= size)
cxrs += convert_ubase(output, num / size, base,
flags, wid - 1, prec - 1);
